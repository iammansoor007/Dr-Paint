...
const Navbar = () => {
  ...
  return (
    <>
      <nav
        className={`fixed top-0 left-0 w-full z-[99999] transition-all duration-300 bg-white border-b border-slate-100 ${scrolled ? "py-2 shadow-xl" : "py-4 shadow-sm"
          }`}
      >
        <div className="max-w-[1400px] mx-auto px-6 md:px-12 flex items-center justify-between">
          ...
          <div className="hidden lg:flex items-center gap-2">
            <div className="relative">
              ...
              >
                ...
                Services
                <ChevronDown
                  className={`h-3 w-3 transition-transform duration-300 ${activeMegaMenu === "services" ? "rotate-180" : ""
                    }`}
                />
              </button>
                  ...
                  >
                    ...
                    <div className="w-64 bg-gradient-to-br from-primary to-primary/80 rounded-[1.5rem] p-8 text-white flex flex-col justify-between overflow-hidden relative group">
                      ...
                      <div className="relative z-10">
                        ...
                        <h3 className="text-2xl font-bold uppercase tracking-tighter leading-none mb-4">
                        ...
                        return (
                          ...
                          >
                            ...
                            <p className="text-[10px] text-muted-foreground leading-relaxed uppercase font-bold tracking-widest">
                              Professional {service.title.split(" ")[0]} solutions for
                              every project.
                            </p>
                          </motion.a>
                        );
          ...
          <div className="flex items-center gap-4">
            <motion.a
              ...
              whileTap={{ scale: 0.95 }}
              className="hidden lg:flex items-center gap-2 bg-primary text-white px-8 py-3 rounded-none text-xs font-bold uppercase tracking-widest shadow-xl shadow-primary/20"
            >
        ...
        {isMenuOpen && (
          <>
            ...
            <motion.div
              ...
              className="fixed inset-0 z-[99998] bg-slate-950/70 backdrop-blur-md lg:hidden"
            />
            ...
            <motion.div
              ...
              className="fixed top-0 right-0 z-[99999] h-full w-[85%] max-w-[400px] bg-white lg:hidden flex flex-col shadow-[-30px_0_60px_rgba(0,0,0,0.3)] overflow-hidden"
            >
              {/* Header */}
              ...
              {/* Navigation Content - ULTRA COMPACT to fit 1 screen */}
              <div className="flex-1 overflow-y-auto px-4 py-6 bg-white">
                <div className="space-y-6">
                  {/* Services Section - 3 Column Grid */}
                  <section>
                    <div className="flex items-center gap-2 mb-4">
                      <p className="text-[9px] font-bold uppercase tracking-[0.2em] text-primary whitespace-nowrap">
                        Services
                      </p>
                    ...
                    <div className="grid grid-cols-3 gap-2">
                      {completeData.services.services.map((s) => (
                        <a
                          ...
                          href="#services"
                          onClick={() => setIsMenuOpen(false)}
                          className="flex flex-col items-center justify-center p-2 rounded-xl border border-slate-50 bg-slate-50/50 active:scale-95 transition-all group"
                        >
                          <div className="h-8 w-8 bg-white rounded-lg flex items-center justify-center text-primary mb-1 shadow-sm group-hover:bg-primary group-hover:text-white transition-colors">
                            {s.icon === "Home" && <Home className="h-4 w-4" />}
                            {s.icon === "Building2" && <Building2 className="h-4 w-4" />}
                            {s.icon === "Wrench" && <Wrench className="h-4 w-4" />}
                            {s.icon === "Hammer" && <Zap className="h-4 w-4" />}
                            {s.icon === "Droplets" && <PaintBucket className="h-4 w-4" />}
                            {s.icon === "Car" && <ShieldCheck className="h-4 w-4" />}
                            {s.icon === "PaintBucket" && <PaintBucket className="h-4 w-4" />}
                          </div>
                          <span className="text-[8px] font-bold uppercase tracking-tight text-slate-900 text-center leading-tight">
                            {s.title.split(" ")[0]}
                          </span>
                        </a>
                      ))}
                  ...
                  {/* Navigation Links Section - Vertical List */}
                  <section>
                    <div className="flex items-center gap-2 mb-4">
                      <p className="text-[9px] font-bold uppercase tracking-[0.2em] text-slate-400 whitespace-nowrap">
                        Quick Menu
                      </p>
                      <div className="h-px flex-1 bg-slate-100" />
                    </div>
                    <div className="flex flex-col gap-1.5">
                      {companyLinks.map((l) => (
                        <a
                          ...
                          href={l.href}
                          onClick={() => setIsMenuOpen(false)}
                          className="flex items-center justify-between p-3.5 rounded-xl text-slate-900 bg-slate-50/30 hover:bg-slate-50 transition-all border border-transparent hover:border-slate-100 group"
                        >
                  ...
                  <section className="pt-4">
                    <a
                      href="#contact"
                      onClick={() => setIsMenuOpen(false)}
                      className="w-full bg-primary text-white py-4 rounded-xl text-center font-bold uppercase tracking-[0.2em] text-[11px] shadow-lg shadow-primary/20 flex items-center justify-center gap-3 active:scale-95 transition-transform"
                    >
                      <Calendar className="h-4 w-4" />
                      Free Quote
                    </a>
